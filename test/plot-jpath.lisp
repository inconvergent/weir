  ...
  (let ((psvg (draw-svg:make* :stroke-width 1d0
    ...
    (let ((res (make-adjustable-vector)))
      ...
      (loop for (c box) across res do
        (draw-svg:jpath psvg (to-list (simplify-path:simplify
                                        (to-vector box :type 'vec:vec)
                                        :lim 1d0))
                        :width 15d0 :closed c)))

    (draw-svg:save psvg (weir-utils:internal-path-string
                          "test/data/plot-jpath"))))

(define-file-tests test-plot-jpath ()
  (time (%main-plot-jpath)))
