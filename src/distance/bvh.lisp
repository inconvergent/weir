...
(defmacro -select-bound ((org invl mi ma sig tmin tmax afx) &body body)
  (alexandria:with-gensyms (ao)
    `(let ((,ao (,afx ,org)))
      (declare (double-float ,ao))
      (multiple-value-bind (,tmin ,tmax)
        (if ,sig (values (* ,invl (- (,afx ,mi) ,ao)) (* ,invl (- (,afx ,ma) ,ao)))
                 (values (* ,invl (- (,afx ,ma) ,ao)) (* ,invl (- (,afx ,mi) ,ao))))
...
(defun -bbox-test (org invx invy invz sigx sigy sigz mi ma)
  (declare #.*opt-settings* (vec:3vec org mi ma)
                            (boolean sigx sigy sigz)
                            (double-float invx invy invz))
  (-select-bound (org invx mi ma sigx tmin tmax vec:3vec-x)
    (-select-bound (org invy mi ma sigy tymin tymax vec:3vec-y)
      (when (or (> tmin tymax) (> tymin tmax)) (return-from -bbox-test nil))
      (when (> tymin tmin) (setf tmin tymin))
      (when (< tymax tmax) (setf tmax tymax))
      (-select-bound (org invz mi ma sigz tzmin tzmax vec:3vec-z)
...
(defun make-line-bbox-test (org l)
  ...
  (multiple-value-bind (invx invy invz) (-eps-inv l)
    (declare (double-float invx invy invz))
    (let ((sigx (<= 0d0 invx))
          (sigy (<= 0d0 invy))
          (sigz (<= 0d0 invz)))
      (declare (boolean sigx sigy sigz))
      (lambda (mi ma) (declare #.*opt-settings* (vec:3vec mi ma))
        (-bbox-test org invx invy invz sigx sigy sigz mi ma)))))
